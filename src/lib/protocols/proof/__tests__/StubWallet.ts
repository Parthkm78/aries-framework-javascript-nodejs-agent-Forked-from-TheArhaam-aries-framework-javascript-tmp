...
export class StubWallet implements Wallet {
  ...
  public proverCreateMasterSecret(masterSecret: string): Promise<string> {
    return Promise.resolve('')
  }
  public proverCreateProof(proofReq: string, requestedCredentials: string, schemas: string, credentialDefs: string, revStates: string): Promise<Proof> {
    return Promise.resolve({
      requested_proof: {},
      proof: {},
      identifiers: [],
  ...
  public verifierVerifyProof(proofReq: string, proof: string, requestedCredentials: string, schemas: string, credentialDefs: string, revocRegDefs: string, revocRegs: string): Promise<boolean> {
    return Promise.resolve(
      true
    )
  }
