...
export class IndyWallet implements Wallet {
   ...
   */
  public async proverCreateProof(proofReq: string, requestedCredentials: string, schemas: string, credentialDefs: string, revStates: string): Promise<Proof> {
    if (!this.wh || !this.masterSecretId) {
      throw Error('Wallet has not been initialized yet');
    }
    // console.log("isnide indy");
    return await this.indy.proverCreateProof(this.wh, proofReq, requestedCredentials, this.masterSecretId, schemas, credentialDefs, revStates);
  }
  ...
  public async proverCreateMasterSecret(masterSecret: string): Promise<string> {
    ...
    return await this.indy.proverCreateMasterSecret(this.wh, masterSecret);
  }
  ...
  public async verifierVerifyProof(proofReq: string, proof: string, requestedCredentials: string, schemas: string, credentialDefs: string, revocRegDefs: string, revocRegs: string): Promise<boolean> {
    if (!this.wh || !this.masterSecretId) {
      throw Error('Wallet has not been initialized yet');
    }
    // console.log("isnide indy");
    return await this.indy.verifierVerifyProof(proofReq, proof, requestedCredentials, schemas, credentialDefs, revocRegDefs, revocRegs);
  }
